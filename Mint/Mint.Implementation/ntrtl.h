...
FORCEINLINE
/* C2040 RtlCopyVolatileMemory: "void (void *,volatile const void *,SIZE_T)" отличается по уровням косвенного обращения от "volatile void
*(volatile void *,volatile const void *,size_t)"
* 
* fixed by just removing memcpy(causes optimization flags to change the function so that volatile just can't do shit) and adding well POINTERS!1
*/
volatile void*
...
{
    // use void* to the byted pointers so that compiler understood step in 1 byte(idk how it works)
    volatile char* dest = (volatile char*)Destination;
    volatile const char* src = (volatile const char*)Source;

    for (size_t i = 0; i < Length; ++i) {
        dest[i] = src[i]; //should generate MOV for every iteration
    }
